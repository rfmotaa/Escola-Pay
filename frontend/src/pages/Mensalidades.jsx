...
export default function Mensalidades() {
  ...
  const [filteredMensalidades, setFilteredMensalidades] = useState([]);
  ...
  const [mensalidadeToDelete, setMensalidadeToDelete] = useState(null);
  ...
  const carregarDados = async () => {
    try {
      ...
      setFilteredMensalidades(mensalidadesData);
    } catch (err) {
  ...
  const handleEditMensalidade = (mensalidade) => {
    setEditingMensalidade(mensalidade);
    setModalOpen(true);
  };

  const handleDeleteClick = (mensalidade) => {
  ...
  const handleMarcarPaga = async (mensalidade) => {
    try {
  ...
  const getPagadorNome = (id_pagador) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="bg-slate-800/50 border-slate-700 backdrop-blur-sm">
            ...
            <CardContent>
              ...
              <p className="text-xs text-slate-500 mt-1">
                {mensalidades.filter((m) => m.status === "pendente").length} pendentes
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-slate-800/50 border-slate-700 backdrop-blur-sm">
            ...
            <CardContent>
              ...
              <p className="text-xs text-slate-500 mt-1">
                {mensalidades.filter((m) => m.status === "paga").length} pagas
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-slate-800/50 border-slate-700 backdrop-blur-sm">
            ...
            <CardContent>
              ...
              <p className="text-xs text-slate-500 mt-1">
                {mensalidades.filter((m) => m.status === "atrasada").length} atrasadas
              </p>
            </CardContent>
          </Card>
        </div>
