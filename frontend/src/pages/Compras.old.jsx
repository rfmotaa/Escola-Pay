...
import { ShoppingCart, Plus, Edit, Trash2, Eye, Search, Calendar, Tag, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function Compras() {
  ...
  // Inicializar com a data atual ao montar
  useEffect(() => {
    if (dataSelecionada) {
      handleDateChange(dataSelecionada);
    }
  }, []);
  ...
  useEffect(() => {
    setCurrentPage(1);
    aplicarFiltros();
  }, [searchTerm, categoriaFilter, compras, dataSelecionada]);
  ...
  const loadComprasUsingDate = () => {
    ...
    setCompraMaisCara(comprasDoMes.length > 0
      ...
      <div className="flex-1 p-4 md:p-6 relative overflow-y-auto">
        <div className="max-w-7xl mx-auto space-y-4 md:space-y-6 pb-20">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-white mb-1 md:mb-2">Gerenciar Compras</h1>
              <p className="text-slate-400 text-sm md:text-base">Controle de compras e despesas do estabelecimento</p>
            </div>
          </div>
        ...
        {filteredCompras.length === 0 ? (
          ...
          <>
            ...
            <p className="text-center text-slate-500 text-sm mt-2">
              Mostrando {startIndex + 1}-{Math.min(startIndex + itemsPerPage, filteredCompras.length)} de {filteredCompras.length} compras
            </p>
          </>
        )}
      </div>
